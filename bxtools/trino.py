...
class TrinoEngine():
    ...
    def __init__(
        ...
        password=None,
        logger='trino',
        query_logger='trino_query_log',
        port=443,
    ):
        
        self.conn_info = {
            #'host': 'dwh-bi.trino.avito.ru',
            'host': 'dwh-trino.k.avito.ru',
            'port': port,
            'user': user,
            'catalog': 'dwh',
            'http_scheme': 'https',
    ...
    def reconnect(self):
        ...
        self.con = connect(**self.conn_info)
    ...
    def insert(self, df, tablename, columns=None):
        ...
        query = f"INSERT INTO {tablename} ({columns_sql}) VALUES ({placeholders})"
        self.query_logger.info(query)
        cur = self.con.cursor()
...
class TrinoIterWorker(TrinoEngine):
    ...
    **kwargs: VericaEngine kwargs
    ...
    def __check_queries_and_params(self):
        ...
        if self.tables_to_swap and self.params.get('ds', None) is None:
            raise TrinoError("For tables swapping you should provide 'ds' (meaning date) key in params")
...
def parse_queries(queries, params=None, delimiter=DFT_QUERIES_DELIMITER, split=True):
    ...
    if params is None:
        ...
        raise TrinoError("'params' variable should be of type 'dict'")
    ...
    if len(queries_list) >= 1:
        ...
        raise TrinoError('Nothing to parse')
...
def parse_tablename(tablename, schema='public'):
    ...
    if len(table_parsed) == 2:
        ...
        raise TrinoError('Incorrect table name' + tablename)
    return table_dict
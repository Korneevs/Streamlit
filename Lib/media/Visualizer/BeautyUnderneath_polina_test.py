...
#                             lambda x: format_with_suffix(x, signed=True, percent=True)
#                       ).astype(str),
                visible=visible,
            ),
            row=1,
...
#         print(fig.data)
    ...
    def show_flight_results(self, df, slices_json, negative_metrics=[], elasticity_dict=None, save=False):
        tr = TableResults()
        title = df['label'].iloc[0]
        fig_table = tr.get_show_tables(df, negative_metrics, elasticity_dict)
...
class TableResults():
    ...
    def get_raw_dataframe(self, ts_df, negative_metrics, elasticity_dict):
...
#         assert len(df['alpha'].unique()) == 1
        ...
        df_display[f' rel. MDE<br>(α={alpha}%)'] = df['rel. MDE'].apply(format_with_suffix, signed=True, percent=True)

        
        if elasticity_dict:
            df_display['ROI'] = df[['ROI lower bound', 'ROI', 'ROI upper bound']]\
                .applymap(format_with_suffix, signed=False, percent=True)\
                .apply(lambda x: self.create_ci_markup(x['ROI lower bound'], 
                                                  x['ROI'],
                                                  x['ROI upper bound']), axis=1)
            df_display['MDE ROI'] = df['MDE ROI'].apply(format_with_suffix, signed=False, percent=True)
        else:
            df_display['ROI'] = 'pass'
            df_display['MDE ROI'] = 'pass'
...
#                          height= self.current_size * 3, 
    ...
    def get_show_tables(self, ts_df, negative_metrics=[], elasticity_dict=None):
        df = self.get_raw_dataframe(ts_df, negative_metrics, elasticity_dict)

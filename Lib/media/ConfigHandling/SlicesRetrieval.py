...
from media.ConfigHandling.SliceInfo import GeneralSliceInfo, MediaSliceInfo
...
class GeneralSlicesRetrieval(SlicesRetrieval):
    ...
    def _create_ch_ids_dict(cls, engine):
        ...
        regIds = engine.select("""SELECT * FROM dct.m42_region""")
        reg_to_id = pd.Series(regIds.id.values, index=regIds.value).to_dict()
        reg_to_id['Any'] = -1

        regIds = engine.select("""SELECT * FROM dct.m42_logical_category""")

        log_to_id = pd.Series(regIds.id.values, index=regIds.value).to_dict()
        log_to_id['Any'] = -1

        vertIds = engine.select("""SELECT * FROM dct.m42_vertical """)
    ...
    def _make_link(cls, main_params, slice_dict, regions, reg_to_id, log_to_id, vert_to_id):
        ...
        link = f"https://m42.k.avito.ru/?&logical_category={log_ids}&metric=1468"\
                    f"&region={reg_ids}&report=main&sum_by=logical_category,region&vertical={vertical_id}"
        return link
    ...
    def get_all_slices_and_info_from_config(cls, 
                                            ...
                                            ch_engine: Optional[CHEngine]=None) -> List:
        """
        ...
        Args:
            main_params:
            slices_json:
            ch_engine: Conection to clickhouse for values validation
        Returns:
            List[Any]: List of slice name and metric pairs
        """
        ...
        reg_to_id, log_to_id, vert_to_id = cls._create_ch_ids_dict(ch_engine)
    ...
    def get_all_slices_and_info_from_config_with_dtb(cls, 
        ...
        for slice_name, metric in slice_metric_pairs:
            ...
            if media_slice_key not in media_slice_keys:
                ...
                corresponding_dtb_slice = media_slice.get_corresponding_dtb_slice(ch_engine)

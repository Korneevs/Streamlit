...
#             print('her')
            ...
            text = ""
            if slices_json is not None and slice_name in slices_json:
                try:
...
#         print(fig.data)
    ...
    def show_flight_results(
        ...
        fig_table = tr.get_show_tables(df, negative_metrics, elasticity_dict, show_mde, adjust_bounds)
...
class TableResults():
    ...
    def __init__(self) -> None:
        ...
        Определяем базовые размеры и цвета.
        Также инициализируем объект для извлечения эффектов.
        """
        # Отключение предупреждений.
        warnings.filterwarnings("ignore")

        # Задание базовых размеров.
        ...
        # Словарь для определения фона в зависимости от знака эффекта.
        self.background_color_to_use = {
            -1: self.background_red_color,
            1: self.background_green_color
        }

        # Словарь для определения цвета текста в зависимости от знака эффекта.
        ...
        # Инициализация объекта для извлечения эффектов.
        self.effectRetrieval = EffectRetrieval()
    
    @staticmethod
    def create_ci_markup(ci_left, upp, ci_right):
        """
        Создает разметку для доверительного интервала (CI).

        Параметры:
        - ci_left (str): левая граница доверительного интервала.
        - upp (str): значение эффекта.
        - ci_right (str): правая граница доверительного интервала.

        Возвращает:
        - str: отформатированная строка с доверительным интервалом и значением эффекта.
        """
        
        # Формируем строку доверительного интервала.
        ci = f"[{ci_left}, {ci_right}]"

        # Вычисляем количество пробелов для центрирования значения эффекта относительно доверительного интервала.
        padding = " " * (int((len(ci) - len(upp)) // 2 * 1.5) + 1)

        # Формируем строку с эффектом, черточками и доверительным интервалом.
        effect = (
            padding + upp + '<br>' +  # Добавляем значение эффекта с отступом и переходом на новую строку.
            "-" * int(len(ci) * 1.5 - 1) + '<br>' +  # Добавляем черточки для визуального разделения.
            ci  # Добавляем доверительный интервал.
        )

        return effect
    
    def get_raw_dataframe(self, ts_df, negative_metrics, elasticity_dict, show_mde, adjust_bounds):
        
        df = self.effectRetrieval.get_effects(ts_df, negative_metrics, elasticity_dict)
        
        df_display = pd.DataFrame()
        
        df_display = pd.DataFrame()
        df_display['slice'] = df['slice']
        df_display['metric'] = df['metric']
        df_display['alternative'] = df['alternative']
        df_display[f"analysed<br>  budget"] = df['analysed budget']\
            .apply(format_with_suffix)
    
        assert len(df['alpha'].unique()) == 1, 'Нужно указать один уровень значимости!'
                          
        alpha = df['alpha'].unique()[0]
       
        print(df)
        
        # Применяем корректировки для эффектов.
        if adjust_bounds:
            for idx, row in df.iterrows():
                if row['rel. effect lower bound'] < 0 and row['rel. effect upper bound'] > 0:
                    if row['metric'] in negative_metrics:
                        df.at[idx, 'rel. effect upper bound'] = 0
                    else:
                        df.at[idx, 'rel. effect lower bound'] = 0
                if row['abs. effect lower bound'] < 0 and row['abs. effect upper bound'] > 0:
                    if row['metric'] in negative_metrics:
                        df.at[idx, 'abs. effect upper bound'] = 0
                    else:
                        df.at[idx, 'abs. effect lower bound'] = 0
        
        print(df)
        
        df_display['rel. effect'] = df[['rel. effect lower bound', 'rel. effect', 'rel. effect upper bound']]\
            .applymap(format_with_suffix, signed=True, percent=True)\
            .apply(lambda x: self.create_ci_markup(x['rel. effect lower bound'], 
                                              x['rel. effect'],
                                              x['rel. effect upper bound']), axis=1)

        df_display['abs. effect'] = df[['abs. effect lower bound', 'abs. effect', 'abs. effect upper bound']]\
            .applymap(format_with_suffix, signed=True, percent=False)\
            .apply(lambda x: self.create_ci_markup(x['abs. effect lower bound'], 
                                              x['abs. effect'],
                                              x['abs. effect upper bound']), axis=1)
        
        if show_mde:
            df_display[f' rel. MDE<br>(α={alpha}%)'] = df['rel. MDE'] \
                .apply(format_with_suffix, signed=True, percent=True)
        
        df_display['method'] = df['method']
        df_display['sign'] = df['sign']
    ...
    def _get_column_fill_text_colors(self, column, signs, start_fill_colors, start_text_colors):
        background = []
        text = []
        if 'effect' in column or 'MDE' in column:
    ...
    def _add_br_in_text(self, text: str) -> str:
        """
        Добавляет тег '<br>' в текст через каждые 15 символов.

        Параметры:
        - text (str): входной текст для обработки.

        Возвращает:
        - str: обработанный текст с добавленными тегами '<br>' (перенос строки).
        """
        size = 15
        res = []
    ...
    def _get_text_height(self, df_col: pd.Series) -> float:
        """
        Вычисляет высоту текста в столбце DataFrame с учетом тегов '<br>' (перенос строки).

        Параметры:
        - df_col (pd.Series): столбец DataFrame для вычисления высоты текста.
    ...
    def _get_html_table(self, df):
        # Извлекаем столбец 'sign' из DataFrame (отвечает за прокрас метрики).
        signs = df['sign']

        # Применяем метод _add_br_in_text к каждому элементу столбцов 'slice' и 'metric'.
        df['slice']  = df['slice'].apply(lambda x: self._add_br_in_text(x))
        df['metric'] = df['metric'].apply(lambda x: self._add_br_in_text(x))

        # Определяем высоту текста в строках столбца 'metric' и 'slice'.
        # Устанавливаем минимальную высоту в 10.
        text_rows_size = max(self._get_text_height(df['metric']), 10)
        text_rows_size = max(self._get_text_height(df['slice']), text_rows_size) + 2

        # Получаем все столбцы DataFrame, кроме последнего.
        columns = df.columns[:-1]

        # Определяем цвета для четных и нечетных строк.
        rowEvenColor = '#f5f5f5'
        rowOddColor = 'white'

        # Устанавливаем начальные цвета заливки и текста для строк.
        ...
        # Задаем размер шрифта заголовков столбцов.
        column_title_size = self.baseline_size + 1

        # Устанавливаем размеры шрифта для ячеек столбцов.
        sizes = [self.baseline_size for column in columns]

        # Определяем параметры заголовков таблицы.
        header = dict(
            values=columns,
            font=dict(family="Arial", size=column_title_size, color='black'),
            fill_color='lightgray',
            line_color='lightgray',
            # Опционально: возможность настройки высоты заголовка.
            # height=header_hight,
            align='center'
        )
        ...
        # Устанавливаем семейство шрифтов для столбцов.
        family = ['Arial' for column in columns]

        # Определяем цвета заливки и текста для ячеек таблицы.
        fill_colors = [
            self._get_column_fill_text_colors(column, signs, start_fill_colors, start_text_colors)['backround']
            for ind, column in enumerate(columns)
        ]
        text_colors = [
            self._get_column_fill_text_colors(column, signs, start_fill_colors, start_text_colors)['text']
            for ind, column in enumerate(columns)
        ]

        # Возвращаем высоту строк текста и объект Table с заголовками и ячейками.
        return text_rows_size, go.Table(
            header=header,
            cells=dict(
                values=values,
                # Опционально: возможность настройки высоты ячеек.
    ...
    def get_show_tables(self, ts_df, negative_metrics=[], elasticity_dict=None, show_mde=False, adjust_bounds=True):
        # Получаем сырые данные в формате DataFrame, используя метод get_raw_dataframe.
        df = self.get_raw_dataframe(ts_df, negative_metrics, elasticity_dict, show_mde, adjust_bounds)

        # Создаем объект Figure с помощью make_subplots, определяя количество строк и столбцов.
        # vertical_spacing и horizontal_spacing устанавливают вертикальные и горизонтальные интервалы.
        # subplot_titles задает заголовок подграфика, specs определяет тип подграфика как "table".
        fig = make_subplots(
            rows=1,
            cols=1,
            vertical_spacing=0.04,
            horizontal_spacing=0.01,
            subplot_titles=[f"Результаты расчета short term (ST) эффекта"],
            specs=[
                [{"type": "table"}],
            ],
        )

        # Извлекаем заголовок из первого элемента столбца 'label' DataFrame ts_df.
        # Это обычно название рекламной кампании, 
        # которое пользователь вводит при указание label в Json с параметрами РегАБ.
        title = ts_df['label'].iloc[0]

        # Определяем отступ сверху (top) в зависимости от наличия '<br>' в заголовке.
        top = 80 if '<br>' in title else 80

        # Получаем высоту и HTML-таблицу, используя метод _get_html_table.
        height, table = self._get_html_table(df)

        # Обновляем макет фигуры, устанавливая различные параметры:
        
        # title:                задает заголовок,
        # title_font:           определяет шрифт заголовка,
        # template:             указывает тему оформления,
        # height:               задает высоту фигуры,
        # width:                задает ширину фигуры,
        # showlegend:           скрывает легенду,
        # legend_tracegroupgap: устанавливает интервал между группами легенды,
        # margin:               определяет отступы (l - левый, r - правый, b - нижний, t - верхний, pad - внутренний отступ).
        ...
        # Добавляем таблицу в фигуру в первую строку и первый столбец.
        fig.add_trace(
            table,
            row=1,
            col=1,

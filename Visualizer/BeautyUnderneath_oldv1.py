...
#         print(fig.data)
    ...
    def show_flight_results(self, df, slices_json, negative_metrics=[], save=False):
        tr = TableResults()
        title = df['label'].iloc[0]
        fig_table = tr.get_show_tables(df, negative_metrics)
...
class TableResults():
    ...
    def make_effect(self, row, eff_type):
...
#     def get_raw_dataframe(self, ts_df, negative_metrics=[]):
#         df = ts_df[ts_df['date'] == ts_df['end_date']]
#         rows = []

#         slices = ts_df['slice_name'].unique()
#         metrics = ts_df['metric'].unique()
        
#         slice_full = set(ts_df['slice_name'] + '@@@' + ts_df['metric'])
        
        
#         for slice_name, metric in beauty_iteration(slice_full):
#             curr = df[(df['metric'] == metric) & (df['slice_name'] == slice_name)]
#             budg = curr['experiment analyzed money'].iloc[0]
...
#                 f'{mde_space} rel. MDE<br>(alpha={alpha}%)': format_with_suffix(row[f'rel. {bound}_error_bound'] * 100, 
...
#         dt = ts_df['end_date'].iloc[0]
        ...
        for slice_name, metric in beauty_iteration(slice_full):
            ...
            rel_row = {
                ...
                'metric': row['metric'],
                "alternative": alternative,
                "effect": self.make_effect(row, eff_type='relative'),
                #format_with_suffix(row[f'rel. effect'] * 100, signed=True, percent=True),
                f'{mde_space}MDE<br>(alpha={alpha}%)': format_with_suffix(row[f'rel. {bound}_error_bound'] * 100, 
                                                                 signed=True, percent=True),
                "sign": sign
            }
            rel_rows.append(rel_row)
            abs_row = deepcopy(rel_row)
#             abs_row["type"] = 'absolute'
            abs_row["effect"] = self.make_effect(row, eff_type='absolute')
            abs_row[f'{mde_space}MDE<br>(alpha={alpha}%)'] = format_with_suffix(row[f'abs. {bound}_error_bound'],
                                                                 signed=True, percent=False)
            abs_rows.append(abs_row)
           
        return {'absolute': pd.DataFrame(abs_rows), 
...
#                          height= self.current_size * 3, 
    ...
    def get_show_tables(self, ts_df, negative_metrics=[]):
        df_json = self.get_raw_dataframe(ts_df, negative_metrics)
#         df = self.get_raw_dataframe(ts_df, negative_metrics)
...
#         )
        fig = make_subplots(
            ...
            vertical_spacing=0.04,
            horizontal_spacing = 0.01,
            subplot_titles=['Relative effects', 'Absolute effects'],
            specs=[
        ...
        height, abs_table = self._get_html_table(df_json['absolute'])
        height, rel_table = self._get_html_table(df_json['relative'])

...
from media.ConfigHandlingTrino.SliceAttributesValidation import SliceAttributesValidation
...
class GeneralSliceInfo(SliceInfo):
    ...
    def validate_attributes(self, engine: Optional[TrinoEngine] = None):
        if self.data_source_type == 'm42':
            assert isinstance(engine, TrinoEngine)
            self.sliceAttributesValidation.check_m42_metric_name_and_is_not_ratio(self.metric, engine)
        self.sliceAttributesValidation.check_dates(self.start_date, self.end_date)


    def get_data_source_type(self, data_source_str):
        if '//ab.k.avito.ru/' in data_source_str:
            return 'm42'
...
class MediaSliceInfo(GeneralSliceInfo):
    ...
    def validate_attributes(self, engine: TrinoEngine):
        if self.data_source_type == 'm42':
            assert isinstance(engine, TrinoEngine)
            self.sliceAttributesValidation.check_m42_metric_name_and_is_not_ratio(self.metric, engine)
        self.sliceAttributesValidation.check_dates(self.start_date, self.end_date)
        self.sliceAttributesValidation.check_vertical(self.vertical, engine)
        self.sliceAttributesValidation.check_logical_category(self.logical_category, engine)
    ...
    def get_corresponding_dtb_slice(self, engine: TrinoEngine) -> 'MediaSliceInfo':
        metric = 'DTB'
        dtb_data_source = self.linkGeneratorM42.get_link(self.vertical, self.logical_category, 
                                                            self.test_regions, [metric], engine)
    ...
    def get_corresponding_federal_dtb_slice(self, engine: TrinoEngine) -> 'MediaSliceInfo':
        metric = 'DTB'
        dtb_data_source = self.linkGeneratorM42.get_link(self.vertical, self.logical_category, 
                                                            ['Any'], [metric], engine)
        return MediaSliceInfo(

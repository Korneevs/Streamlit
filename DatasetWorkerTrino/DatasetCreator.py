...
from media.DatasetWorkerTrino.M42DatasetCreator import M42DatasetCreator 
from media.DatasetWorkerTrino.execSQLWorker import execSQLWorker
from media.ConfigHandlingTrino.RegionsRetrieval import RegionsRetrieval
from media.ConfigHandlingTrino.RegionsRetrieval import RegionsRetrieval
from media.ConfigHandlingTrino.SlicesRetrieval import GeneralSlicesRetrieval, MediaSlicesRetrieval
from media.ConfigHandlingTrino.MetaInfo import MetaInfo
from media.DatasetWorkerTrino.ConfigToQuerySlice import ConfigToQuerySlice, MediaConfigToQuerySlice
from media.DatasetWorkerTrino.QuerySliceInfo import QuerySliceInfo
from media.DatasetWorkerTrino.drop_regions import drop_regions as DROP_REGIONS
...
class DatasetCreator():
    ...
    def __init__(self, t_engine, for_media=True) -> None:
        """
    ...
    def check_values(self, slice_dict, value):
        ...
        and is_active=true
        """
        good_names = set(self.engine.select(query)['value'])
...
#             raise DatasetWorkerError(f"Даты анализа {analyse_date} нет в полученном датасете. Проверьте дату!")
    ...
    def get_datasets(self, main_params, slices_json):
        ...
        config_slices_info_list = self.slicesRetrievalMethod(
            main_params=main_params, slices_json=slices_json, engine=self.engine)
        ...
        for config_slice in config_slices_info_list:
            config_slice.validate_attributes(self.engine)
